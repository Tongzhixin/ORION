...
class MergeAll(DActor):
    ...
    def sink(self, input_table: pa.Table):
        print(f"table size:{input_table.nbytes/1024}KB")
        del input_table

    def run(self, input_table: pa.Table):
        s1 = self.projection(input_table)
        s2 = self.agg_pyarrow(s1)
        self.sink(s2)
        del s1
        del s2
        del input_table
...
def main():
    ...
    merge_actor = MergeAll.options(max_concurrency=40).remote(
            group_keys, aggregate_elements, agg_run_type, projectionElement_list
        )
    for file_name in exist_files:
        if len(ref_list) > max_cpu:
            ready_refs, ref_list = ray.wait(ref_list, num_returns=4)
            ray.get(ready_refs)
        table_ref = table_scan.remote(bucket, file_name, [])
        
        next_ref = merge_actor.run.remote(table_ref)
