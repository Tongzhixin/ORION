...
ray.init(
    ...
    object_store_memory=28474836480,
...
@ray.remote
class LRUCache:
    def __init__(self, max_size=24 * 1024):
        self.max_size = max_size
        self.current_size = 0
        self.cache = dict()  # stores the key and ray object ref
...
class S3FileReader:
    ...
    def process(self, columns):
        ...
        for column in columns:
            ...
            ray.get(self.cache.put.remote(f"{self.key}.{column}", col_data))
        return table
    ...
    def table_scan(self, columns=None, filters=None):
        ...
        cached_keys: list = ray.get(self.cache.get_cached_key.remote())
        # print(cached_keys)
        ...
        exist_column_data, miss_columns = ray.get(
            self.cache.get_cached_value.remote(
                [f"{self.key}.{col}" for col in exist_columns]
            )
        )
